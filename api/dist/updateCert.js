"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.updateCert = updateCert;
const functions_1 = require("@azure/functions");
const cosmos_1 = require("./utils/cosmos");
const status_1 = require("./utils/status");
const auth_1 = require("./utils/auth");
const audit_1 = require("./utils/audit");
const http_1 = require("./utils/http");
function updateCert(request, context) {
    return __awaiter(this, void 0, void 0, function* () {
        context.log(`Http function processed request for url "${request.url}"`);
        try {
            const userCtx = (0, auth_1.getUserContext)(request);
            if (!userCtx)
                return http_1.ERR.UNAUTHORIZED();
            const { districtId, roles } = userCtx;
            if (!districtId)
                return http_1.ERR.NO_DISTRICT();
            if (request.method !== 'PATCH' && request.method !== 'POST')
                return http_1.ERR.METHOD_NOT_ALLOWED();
            const url = new URL(request.url);
            const id = url.searchParams.get('id');
            if (!id)
                return http_1.ERR.VALIDATION('Missing id');
            if (!(0, auth_1.authorizeRole)(roles, ['district_admin', 'school_admin'])) {
                return http_1.ERR.FORBIDDEN();
            }
            const { resources } = yield cosmos_1.entities.items
                .query({
                query: 'SELECT * FROM c WHERE c.type = @t AND c.id = @id AND c.districtId = @did',
                parameters: [
                    { name: '@t', value: 'cert' },
                    { name: '@id', value: id },
                    { name: '@did', value: districtId },
                ],
            })
                .fetchAll();
            if (resources.length === 0)
                return http_1.ERR.NOT_FOUND('Cert not found');
            const cert = resources[0];
            // If school_admin, ensure school scope
            if (!roles.has('district_admin') && roles.has('school_admin')) {
                const profile = yield (0, auth_1.getUserProfile)(districtId, userCtx.userId);
                if ((profile === null || profile === void 0 ? void 0 : profile.schoolId) && profile.schoolId !== cert.schoolId) {
                    return http_1.ERR.FORBIDDEN('Forbidden for this school');
                }
            }
            const body = (yield request.json());
            const updated = Object.assign({}, cert);
            if (typeof body.issueDate !== 'undefined')
                updated.issueDate = body.issueDate;
            if (typeof body.expiryDate !== 'undefined')
                updated.expiryDate = body.expiryDate;
            if (typeof body.docPath !== 'undefined')
                updated.docPath = body.docPath;
            if (typeof body.certTypeKey !== 'undefined')
                updated.certTypeKey = body.certTypeKey;
            if (updated.expiryDate)
                updated.status = (0, status_1.computeStatus)(updated.expiryDate);
            updated.updatedAt = new Date().toISOString();
            const { resource } = yield cosmos_1.entities.items.upsert(updated);
            yield (0, audit_1.auditLog)(districtId, userCtx.userId, 'update', 'cert', updated.id, { changed: Object.keys(body || {}) });
            return (0, http_1.ok)(resource);
        }
        catch (error) {
            context.log('Error updating cert:', error);
            return http_1.ERR.INTERNAL();
        }
    });
}
functions_1.app.http('updateCert', {
    methods: ['PATCH', 'POST'],
    authLevel: 'anonymous',
    handler: updateCert,
});
//# sourceMappingURL=updateCert.js.map