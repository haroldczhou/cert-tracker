...
function approveEmailChange(request, context) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const people = yield (0, cosmos_1.queryAll)({ query: 'SELECT TOP 1 * FROM c WHERE c.type = @t AND c.id = @pid AND c.districtId = @did', parameters: [{ name: '@t', value: 'person' }, { name: '@pid', value: reqDoc.personId }, { name: '@did', value: districtId }] });
            if (people.length === 0)
                return http_1.ERR.NOT_FOUND('Person not found');
            const person = people[0];
            person.email = reqDoc.newEmail;
            person.updatedAt = new Date().toISOString();
            yield cosmos_1.entities.items.upsert(person);
            reqDoc.status = 'approved';
            reqDoc.updatedAt = new Date().toISOString();
            yield cosmos_1.entities.items.upsert(reqDoc);
            return (0, http_1.ok)({ approved: true });
        }
        catch (e) {
            context.log('approveEmailChange error', e);
            return http_1.ERR.INTERNAL();
        }
    });
}
functions_1.app.http('approveEmailChange', {
    methods: ['POST'],
    authLevel: 'anonymous',
    handler: approveEmailChange,
});
//# sourceMappingURL=approveEmailChange.js.map