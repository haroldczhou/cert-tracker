...
export async function approveEmailChange(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  try {
    ...
    const people = await queryAll<any>({ query: 'SELECT TOP 1 * FROM c WHERE c.type = @t AND c.id = @pid AND c.districtId = @did', parameters: [{ name: '@t', value: 'person' }, { name: '@pid', value: reqDoc.personId }, { name: '@did', value: districtId }] });
